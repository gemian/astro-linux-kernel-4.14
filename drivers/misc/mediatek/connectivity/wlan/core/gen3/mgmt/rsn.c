...
{
	UINT_8 selfMfpc, selfMfpr, peerMfpc, peerMfpr;
	...
	if ((selfMfpc == TRUE) && (peerMfpc == TRUE)) {
		DBGLOG(RSN, INFO, "PMF Connection\n");
 ...
 */
/*----------------------------------------------------------------------------*/
uint8_t rsnApCheckSaQueryTimeout(IN P_ADAPTER_T
				 prAdapter, IN P_STA_RECORD_T prStaRec)
{
	P_BSS_INFO_T prBssInfo;
	uint32_t now;
 ...
 */
/*----------------------------------------------------------------------------*/
void rsnApStopSaQuery(IN P_ADAPTER_T prAdapter,
		      IN P_STA_RECORD_T prStaRec)
{
	cnmTimerStopTimer(prAdapter, &prStaRec->rPmfCfg.rSAQueryTimer);
	prStaRec->rPmfCfg.u2TransactionID = 0;
 ...
 */
/*----------------------------------------------------------------------------*/
void rsnApSaQueryRequest(IN P_ADAPTER_T prAdapter,
			 IN P_SW_RFB_T prSwRfb)
{
	...
	uint16_t u2PayloadLen;
	P_STA_RECORD_T prStaRec;
	P_ACTION_SA_QUERY_FRAME prTxFrame = NULL;
	...
	prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, prStaRec->ucBssIndex);
	prRxFrame = (P_ACTION_SA_QUERY_FRAME)
	    prSwRfb->pvHeader;
	if (!prRxFrame)
		return;
