...
extern unsigned char IsAeonHdmi;
	...
	|| defined(CONFIG_TOUCHSCREEN_MTK_SYNAPTICS_3320_50) || defined(CONFIG_TOUCHSCREEN_FTS)
...
void tpd_get_dts_info(void)
{
	...
	if (node1) {
		...
		printk("can't find touch compatible custom node\n");
	}
}
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	if (tpd_dts_data.tpd_use_ext_gpio == false) {
		...
		if (IS_ERR(rst_output0)) {
			...
			printk("Cannot find pinctrl state_rst_output0!\n");
			return ret;
		}
		rst_output1 =
		...
		if (IS_ERR(rst_output1)) {
			...
			printk("Cannot find pinctrl state_rst_output1!\n");
			return ret;
		}
	}
	printk("[tpd%d] mt_tpd_pinctrl----------\n", pdev->id);
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case TPD_GET_FILTER_PARA:
			...
			if (data == NULL) {
				err = -EINVAL;
				printk("GET_FILTER_PARA: data is null\n");
				break;
			}

			if (copy_to_user(data, &tpd_filter,
					sizeof(struct tpd_filter_t))) {
				printk("GET_FILTER_PARA: copy data error\n");
				err = -EFAULT;
				break;
			}
			break;
			...
			unsigned long event, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		...
		if (g_tpd_drv && tpd_suspend_flag) {
			err = queue_work(touch_resume_workqueue,
						&touch_resume_work);
			if (!err) {
				printk("start resume_workqueue failed\n");
				return err;
			}
		}
		break;
	case FB_BLANK_POWERDOWN:
		...
		if (g_tpd_drv && !tpd_suspend_flag) {
			...
			if (!err)
				printk("cancel resume_workqueue failed\n");
...
static void tpd_create_attributes(struct device *dev, struct tpd_attrs *attrs)
{
	...
	for (; num > 0;) {
		if (device_create_file(dev, attrs->attr[--num]))
			printk("mtk_tpd: tpd create attributes file failed\n");
	}
}
...
/*for some projects, we do not use mtk framebuffer*/
	...
	TPD_RES_Y = tpd_dts_data.tpd_resolution[1];
		...
		if (ret < 0) {
			printk("Touch down get lcm_x failed");
			return ret;
		}
		TPD_RES_X = tpd_res_x;
		ret = kstrtoul(CONFIG_LCM_HEIGHT, 0, &tpd_res_x);
		if (ret < 0) {
			printk("Touch down get lcm_y failed");
			return ret;
		}
		TPD_RES_Y = tpd_res_y;
#endif
	...
	if (g_tpd_drv == NULL) {
		if (tpd_driver_list[0].tpd_device_name != NULL) {
			...
			printk("Generic touch panel driver\n");
		} else {
			printk("no touch driver is loaded!!\n");
			return 0;
		}
	}
	...
	if (input_register_device(tpd->dev))
		printk("input_register_device failed.(tpd)\n");
	else
		tpd_register_flag = 1;
...
{
	printk("[tpd] MediaTek touch panel driver init\n");
...
static int __init tpd_device_init(void)
{
	...
	if (!res)
		printk("tpd : touch device init failed res:%d\n", res);
	return 0;
}
