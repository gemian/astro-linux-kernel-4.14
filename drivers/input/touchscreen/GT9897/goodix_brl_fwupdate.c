...
#define FW_HEADER_SIZE_BRB				512
#define FW_SUBSYS_INFO_SIZE				10
#define FW_SUBSYS_INFO_OFFSET_BRA		36
#define FW_SUBSYS_INFO_OFFSET_BRB		42
...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	int subsys_info_offset = goodix_fw_update_ctrl.update_info.subsys_info_offset;
...
static int goodix_fw_version_compare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (fwu_ctrl->ic_config && fwu_ctrl->ic_config->len > 0) {
		...
		goodix_reg_read(config_id_reg, (u8 *)&ic_cfg_id, sizeof(ic_cfg_id));
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	u8 enable_misctl_val = fwu_ctrl->update_info.enable_misctl_val;
	u8 reg_val[4] = {0};
	u8 temp_buf[64] = {0};
	int retry = 20;
	int r;
		...
		const char *buf, size_t count)
{
	...
	struct goodix_ic_config *cfg =
			fw_ctrl->core_data->ic_configs[CONFIG_TYPE_NORMAL];
	...
	ret = goodix_do_fw_update(cfg, mode);
	if (!ret) {
...
int goodix_fw_update_init(struct goodix_ts_core *core_data)
{
	...
	if (core_data->bus->ic_type == IC_TYPE_BERLIN_A) {
		goodix_fw_update_ctrl.update_info.header_size = FW_HEADER_SIZE_BRA;
		goodix_fw_update_ctrl.update_info.subsys_info_offset = FW_SUBSYS_INFO_OFFSET_BRA;
		goodix_fw_update_ctrl.update_info.isp_ram_reg = ISP_RAM_ADDR_BRA;
		goodix_fw_update_ctrl.update_info.flash_cmd_reg = FLASH_CMD_REG_BRA;
		goodix_fw_update_ctrl.update_info.isp_buffer_reg = HW_REG_ISP_BUFFER_BRA;
		goodix_fw_update_ctrl.update_info.config_data_reg = CONFIG_DATA_ADDR_BRA;
		goodix_fw_update_ctrl.update_info.misctl_reg = MISCTL_REG_BRA;
		goodix_fw_update_ctrl.update_info.watch_dog_reg = WATCH_DOG_REG_BRA;
		goodix_fw_update_ctrl.update_info.enable_misctl_val = ENABLE_MISCTL_BRA;
		goodix_fw_update_ctrl.update_info.config_id_reg = GOODIX_CFG_ID_ADDR_BRA;
	} else {
		goodix_fw_update_ctrl.update_info.header_size = FW_HEADER_SIZE_BRB;
		goodix_fw_update_ctrl.update_info.subsys_info_offset = FW_SUBSYS_INFO_OFFSET_BRB;
		goodix_fw_update_ctrl.update_info.isp_ram_reg = ISP_RAM_ADDR_BRB;
		goodix_fw_update_ctrl.update_info.flash_cmd_reg = FLASH_CMD_REG_BRB;
		goodix_fw_update_ctrl.update_info.isp_buffer_reg = HW_REG_ISP_BUFFER_BRB;
		goodix_fw_update_ctrl.update_info.config_data_reg = CONFIG_DATA_ADDR_BRB;
		goodix_fw_update_ctrl.update_info.misctl_reg = MISCTL_REG_BRB;
		goodix_fw_update_ctrl.update_info.watch_dog_reg = WATCH_DOG_REG_BRB;
		goodix_fw_update_ctrl.update_info.enable_misctl_val = ENABLE_MISCTL_BRB;
		goodix_fw_update_ctrl.update_info.config_id_reg = GOODIX_CFG_ID_ADDR_BRB;
	}
