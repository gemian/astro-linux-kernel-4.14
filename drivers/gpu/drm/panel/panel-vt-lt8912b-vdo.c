...
extern int FUSB300Write(unsigned char regAddr, unsigned char length, unsigned char *data);
extern int FUSB300Read(unsigned char regAddr, unsigned char length, unsigned char *data);
extern int aeon_gpio_set(const char *name);
extern unsigned int aeon_gpio_get(const char *name);
extern void lt8912_suspend(void);
extern void lt8912_resume(void);
unsigned char IsAeonHdmi = 0;
...
#define VAC (480)
#define HAC (854)
static u32 fake_heigh = 480;
static u32 fake_width = 854;
#else
#define HFP (110)
#define HSA (40)
#define HBP (220)
...
{
	char bl_tb0[] = {0x51, 0x3, 0xff};

	if (level > 255)
		level = 255;

	level = level * 1023 / 255;
	bl_tb0[1] = ((level >> 8) & 0x3);
	bl_tb0[2] = (level & 0xff);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	printk("lcm_probe start lt8912b\n");
	...
	if ((get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT || get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT) 
		...
		FUSB300Read(0x11, 1, &CCXstate);	/* Read CC1 CC2 state*/
	...
	/*dsi->mode_flags = MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_BURST
		...
		| MIPI_DSI_MODE_VSYNC_FLUSH | MIPI_DSI_MODE_VIDEO_AUTO_VERT | MIPI_DSI_MODE_VIDEO_SYNC_PULSE;*/
	dsi->mode_flags = MIPI_DSI_MODE_VIDEO
		| MIPI_DSI_MODE_VIDEO_HFP | MIPI_DSI_MODE_VIDEO_HBP
		| MIPI_DSI_MODE_VIDEO_HSA | MIPI_DSI_MODE_EOT_PACKET
...
MODULE_DESCRIPTION("VT DR6110 VDO LCD Panel Driver");
